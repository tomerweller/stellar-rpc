...
func TestGetEventsTopicANDLogic(t *testing.T) {
	...
	err = eventReader.GetEvents(ctx, cursorRange, nil, topics, nil, EventOrderAsc, scanFunc)
	require.NoError(t, err)
	...
	err = eventReader.GetEvents(ctx, cursorRange, nil, topicsOnlyTransfer, nil, EventOrderAsc, scanFunc)
	require.NoError(t, err)
	...
	err = eventReader.GetEvents(ctx, cursorRange, nil, topicsOnlyAddressA, nil, EventOrderAsc, scanFunc)
	require.NoError(t, err)
